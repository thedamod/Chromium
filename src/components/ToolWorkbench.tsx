...
export default function ToolWorkbench() {
  ...
  const [icoStatus, setIcoStatus] = useState('Upload a source image to generate a favicon-ready ICO file.')
  const [icoError, setIcoError] = useState('')

  const [markdown, setMarkdown] = useState(defaultMarkdown)
  const [pdfStatus, setPdfStatus] = useState('Ready to export.')

  const [formatSource, setFormatSource] = useState<ImageSourceState | null>(null)
  ...
  const [formatStatus, setFormatStatus] = useState('Choose a source image and target format.')
  ...
  try {
    ...
    regexError = error instanceof Error ? error.message : 'The DSL could not be compiled.'
  }
  ...
  async function handleBase64Upload(file: File | null) {
    ...
    try {
      ...
      setBase64Error(error instanceof Error ? error.message : 'Unable to read the image.')
    }
  }
  ...
  async function handleDecodeBase64() {
    try {
      ...
      setBase64DecodedName(`decoded.${extensionFromMime(blob.type || base64MimeHint)}`)
      setBase64Error('')
    } catch (error) {
  ...
  async function handleCreateIco(file: File | null) {
    ...
    try {
      ...
      setIcoError(error instanceof Error ? error.message : 'ICO conversion failed.')
    }
  }
  ...
  async function handleConvertImage() {
    ...
    try {
      ...
      setFormatOutputName(`${fileNameWithoutExtension(formatSource.filename)}.${extension}`)
      setFormatStatus(
        `Converted ${formatSource.filename} to ${extension.toUpperCase()} at ${canvas.width}x${canvas.height}.`,
      )
      setFormatError('')
    } catch (error) {
      setFormatError(error instanceof Error ? error.message : 'Image conversion failed.')
    }
  }
  ...
  return (
    <section className="mt-8 space-y-6">
      <div className="grid gap-4 xl:grid-cols-2">
        <article id="tool-grid" className="tool-card rounded-[1.7rem] p-5 sm:p-6">
          <SectionHeading
            icon={<Binary size={18} />}
            title="Image <> Base64"
          ...
          <div className="tool-layout">
            <div className="tool-surface">
              <label className="tool-label">
                ...
                <input
                  ...
                  type="file"
                  accept="image/*"
                  onChange={(event) => void handleBase64Upload(event.target.files?.[0] ?? null)}
                />
              </label>
              {base64Image ? (
            ...
            <div className="tool-surface">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button type="button" className="tool-button tool-button--quiet" onClick={() => void handleDecodeBase64()}>
                  <RotateCw size={16} />
              ...
              {base64DecodedUrl ? (
                <div className="mt-4 space-y-3">
                  <img src={base64DecodedUrl} alt="" className="tool-preview-image" />
                  <a className="tool-button" href={base64DecodedUrl} download={base64DecodedName}>
                    <Download size={16} />
          ...
          <div className="tool-layout">
            <div className="tool-surface">
              <label className="tool-label">
                ...
                <input
                  ...
                  type="file"
                  accept="image/*"
                  onChange={(event) => void handleCreateIco(event.target.files?.[0] ?? null)}
                />
              </label>
              {icoSource ? (
                <>
                  ...
                  <img src={icoSource.dataUrl} alt="" className="tool-preview-image" />
                </>
              ) : (
            ...
            <div className="tool-surface">
              ...
              <p className="tool-caption">
                This exporter wraps a 256x256 PNG payload inside an ICO container, which works well for modern favicon and desktop icon use cases.
              </p>
              ...
              <button
                ...
                onClick={() => {
                  if (icoReady && icoSource) {
                    downloadBlob(icoReady, `${fileNameWithoutExtension(icoSource.filename)}.ico`)
                  }
                }}
              >
                <Download size={16} />
            ...
            <div className="tool-surface">
              ...
              {formatSource ? (
                <>
                  ...
                  <img src={formatSource.dataUrl} alt="" className="tool-preview-image" />
                </>
              ) : (
            ...
            <div className="tool-surface">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="tool-label">
                  ...
                  />
                  <span className="tool-caption">{Math.round(formatQuality * 100)}%</span>
                </label>
              </div>
              <div className="mt-4 flex flex-wrap gap-3">
                <button type="button" className="tool-button" onClick={() => void handleConvertImage()}>
                  <WandSparkles size={16} />
                  Convert image
                </button>
                {formatOutputUrl ? (
                  <a className="tool-button tool-button--quiet" href={formatOutputUrl} download={formatOutputName}>
                    <Download size={16} />
                    Download output
                  </a>
                ) : null}
              ...
              {formatOutputUrl ? (
                <div className="mt-4 space-y-3">
                  <img src={formatOutputUrl} alt="" className="tool-preview-image" />
      ...
      <article className="tool-card rounded-[1.7rem] p-5 sm:p-6">
        ...
        <div className="tool-layout">
          <div className="tool-surface">
            ...
            <p className="tool-caption">
              Supported functions: `and`, `or`, `not`, `group`, `optional`, `zeroOrMore`, `oneOrMore`, `repeat`, `literal`, `startsWith`, `endsWith`, `digit`, `digits`, `word`, `whitespace`, `any`, `charIn`, `charRange`.
            </p>
          </div>
          ...
          <div className="tool-surface">
            ...
            {regexError ? (
              ...
              <>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <button type="button" className="tool-button" onClick={() => void copyText(regexOutput)}>
                    <Copy size={16} />
                    Copy regex
                  </button>
                  <p className={`tool-status ${regexMatches ? 'text-[var(--palm)]' : ''}`}>
                    {regexMatches
