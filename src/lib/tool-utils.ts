	...
	switch (node.name) {
		...
		case "repeat": {
			if (node.args.length < 2 || node.args.length > 3) {
				throw new Error(
					"repeat() expects a pattern, a min, and an optional max.",
				);
			}

			const pattern = compileAst(node.args[0]);
			const min = expectNumberArg(node.args[1], "repeat");
			const max = node.args[2]
				? expectNumberArg(node.args[2], "repeat")
				: undefined;
		...
		case "literal":
			...
			return escapeRegex(
				(node.args[0] as Extract<AstNode, { type: "string" }>).value,
			);
		case "startsWith":
			ensureArgCount(node.name, args, 1);
			return `^${args[0]}`;
...
function ensureMinArgs(name: string, args: string[], count: number) {
	if (args.length < count) {
		throw new Error(
			`${name}() expects at least ${count} argument${count === 1 ? "" : "s"}.`,
		);
	}
}

function ensureStringArgCount(
	node: Extract<AstNode, { type: "call" }>,
	count: number,
) {
	if (
		node.args.length !== count ||
		node.args.some((arg) => arg.type !== "string")
	) {
		throw new Error(
			`${node.name}() expects ${count} string argument${count === 1 ? "" : "s"}.`,
		);
	}
}
